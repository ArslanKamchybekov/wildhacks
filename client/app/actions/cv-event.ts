...
export async function generateRoast(
  ...
  try {
    ...
    let alignmentReason = "";
    let isEmptyUrl =
      !current_tab_url || current_tab_url === "" || current_tab_url === "null";
    ...
    if (!isEmptyUrl && activeSession && activeSession.goal) {
      ...
      try {
        ...
        if (response.startsWith("yes")) {
          ...
          alignmentReason = response.includes("(")
    ...
    if (shouldRoast || isEmptyUrl) {
      ...
      if (isEmptyUrl) {
        console.log(
          "Generating roast based on computer vision data (no URL)..."
        );
      } else {
        console.log(
          "URL does not align with session goal. Generating roast..."
        );
      }
